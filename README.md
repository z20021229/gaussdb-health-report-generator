...
5. 定义统一 YAML schema，承载客户、实例、节点、指标、风险和附录路径。
6. 开发通用解析器，避免依赖单一客户文件名、单一 IP 或单一节点数量。
7. 实现风险分析规则，将解析结果转换为风险等级、问题说明和整改建议。
8. 基于 Word 样例格式生成正式 GaussDB 数据库健康诊断报告。
9. 扩展测试体系，覆盖解析成功、字段缺失、多节点、多格式和异常输入场景。
10. 持续提升解析覆盖率，并保持工具在不完整输入下仍可运行。
